    ...
    <section className="mt-5 rounded-xl border border-border bg-background">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3 sm:px-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Ícone e informações cadastrais</p>
        </div>
        {canEdit ? (
          <button
            type="button"
            onClick={onEditar}
            className="rounded-lg border border-border bg-surface-elevated px-3 py-1.5 text-sm text-foreground hover:bg-muted"
          >
            Editar dados
          </button>
        ) : null}
