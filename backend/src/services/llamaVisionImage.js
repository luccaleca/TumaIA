 ...
 * @param {{ maxBytes?: number, timeoutMs?: number, retries?: number }} [opts]
...
export async function fetchImageBuffer(imageUrl, opts = {}) {
  ...
  const timeoutMs = Math.max(1_000, Number(opts?.timeoutMs) || FETCH_TIMEOUT_MS);
  const retries = Math.max(0, Math.min(3, Math.round(Number(opts?.retries) || 0)));
    ...
    try {
      ...
      const nextErr =
        err instanceof Error && err.name === "AbortError"
          ? new Error("Tempo esgotado ao baixar a imagem do acervo.")
          : err;
      if (attempt >= retries) throw nextErr;
